...
```{r}
gss16_test_predictions <- predict(gss16_fit_1, gss16_test, type = "prob") %>%
...
results_1 <- gss16_test_predictions %>%
  roc_auc(
...
```{r}
gss16_test_predictions_1 <- predict(gss16_fit_1, gss16_test, type = "prob") %>%
  bind_cols(gss16_test %>%
              select(advfront))
gss16_test_predictions_1 <- gss16_test_predictions %>%
  mutate(pred_class = factor(
    if_else(.pred_Agree > 0.83, "Agree", "Not agree"),
    levels = c("Not agree", "Agree")
  ))
conf_mat <- conf_mat(
  gss16_test_predictions_1,
  truth = advfront,
  estimate = pred_class
)
conf_mat

summary_1 <- summary(conf_mat_1)
summary_1 %>%
  filter(.metric == "sens") %>%
  pull(.estimate)
summary_1 %>%
...
```{r}
gss16_rec_2 <- recipe(advfront ~ polviews + wrkstat + emailhr + educ, data = gss16_train) %>%
...
gss16_predictions_1 <- gss16_fit_2 %>%
  predict(gss16_test, type = "prob") %>%
  bind_cols(gss16_test)

roc_curve_1 <- gss16_predictions_1 %>%
  roc_curve(
    truth = advfront, .pred_Agree, event_level = "second"
  )
autoplot(roc_curve_1)

auc_results <- gss16_predictions_1 %>%
  roc_auc(
    truth = advfront, .pred_Agree, event_level = "second"
  )
auc_results
...
The model that performs better is model 1, gss16_fit_1. This is because there is more area under the curve in model 1 as compared to model 2, thus model 1 is more accurate. This is because the higher the AUC, the more significance the results of the model are. Thus because the AUC of model 1 is greater, the data is more meaningful. 
